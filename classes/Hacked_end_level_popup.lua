...
local newHacked_Shade = require('classes.Hacked_shade').newHacked_Shade

function _M.newHacked_EndLevelPopup(params)
	...
	local restartButton = widget.newButton({
		...
		onRelease = function()
			sounds.play('tap')
			composer.gotoScene('scenes.Hacked_reload_game', {params = params.levelId})
		end
	})
	...
	if params.levelId < composer.getVariable('levelCount') then
		local nextButton = widget.newButton({
			...
			onRelease = function()
				sounds.play('tap')
				composer.gotoScene('scenes.Hacked_reload_game', {params = params.levelId + 1})
			end
		})
	...
	function popup:show(params)
		...
		self.Hacked_shade = newHacked_Shade(superParams.g)
		self:toFront()
